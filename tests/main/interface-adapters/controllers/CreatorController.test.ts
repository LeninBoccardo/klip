...
describe('CreatorController', () => {
  ...
  it('registers all four creator channels', () => {
    registerCreatorController(makeRepo())
    expect([...electron.handlers.keys()].sort()).toEqual(
      ['delete-creator', 'get-creator-by-id', 'get-creators-paginated', 'restore-creator'].sort()
    )
  })

  it('"get-creators-paginated" forwards params to findPaginated', async () => {
    const repo = makeRepo()
    registerCreatorController(repo)
